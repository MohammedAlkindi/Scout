...
4. Start a new scout from a city destination such as `Dubai` and verify the
   session uses that destination label before running an activity.
5. Copy or open the read-only share link and verify it recreates the scout
   result without signing in.
6. Start a live scout with manual coordinates near Muscat
   (`23.5793, 58.4025`) and activity `Coastal sunset`.
7. If a provider is slow, the Muscat demo-style request should return
